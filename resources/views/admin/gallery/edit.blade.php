            ...
            <form action="{{ route('gallery.update', $galleryItem->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    @if ($galleryItem->path_gambar)
                        <div class="mb-2">
                            <img src="{{ asset('uploads/gallery/' . $galleryItem->path_gambar) }}" alt="Gambar Saat Ini" style="width: 150px; height: 100px; object-fit: cover; border-radius: 4px;">
                            <small class="form-text text-muted d-block">Gambar saat ini.</small>
                        </div>
                    @endif
                    
                    <input type="file" name="path_gambar" class="form-control @error('path_gambar') is-invalid @enderror">
                    @error('path_gambar')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    <small class="form-text text-muted">Biarkan kosong jika tidak ingin mengganti gambar. Format: Gambar (max 2MB)</small>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="deskripsi_singkat" class="form-control @error('deskripsi_singkat') is-invalid @enderror" rows="3" required>{{ old('deskripsi_singkat', $galleryItem->deskripsi_singkat) }}</textarea>
